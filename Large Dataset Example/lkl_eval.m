function [xdet,xdet_d,energy,energy_d,sol_out,A_d_sol]=lkl_eval(nodes,y,params)
...
if size(y,2)==1
    ...
    floor_cut=max(1,min(10*floor(cutoff_size/10),floor_cut));
    params(6)={floor_cut};
    
    [ind_perm]=perm_generator(nodes,{kernel_params{1},kernel_params{2},0},cutoff_size,[1:size(nodes,2)]);
...
[offD_USV,offD_USVd,offD_dUSV]=compute_offD_lkl(nodes,indices_agg,kernel_params,k_min);

[xdet,xdet_d,energy,energy_d,sol_out,A_d_sol]=SMW_lkl(indices_agg,offD_USV,offD_USVd,offD_dUSV,nodes,y,params);

if size(y,2)==1
    sol_out_mat=cell2mat(sol_out);
    sol_out_mat_perm(ind_perm,:)=sol_out_mat;
    sol_out=sol_out_mat_perm;
    A_d_sol_mat_perm=zeros(size(sol_out_mat_perm,1),size(sol_out_mat_perm,2));
    for j=1:length(A_d_sol)
        A_d_solj=A_d_sol{j};
        A_d_sol_mat=cell2mat(A_d_solj);
        A_d_sol_mat_perm(ind_perm,:)=A_d_sol_mat;
        A_d_sol(j)={A_d_sol_mat_perm};
    end
end
